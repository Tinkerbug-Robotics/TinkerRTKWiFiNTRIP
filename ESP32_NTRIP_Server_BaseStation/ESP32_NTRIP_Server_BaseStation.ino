/**
 * Firmware for ESP32 running as a base station sending correction data to an NTRIP caster.
 * This firmware displays information about the base station and data transmitted to a webpage,
...
// NTRIP server and connection attempt parameters
NTRIPServer ntrip_s;
int initialTimeout = 30000;
unsigned long ntripServerLastConnectionAttempt = 0;
unsigned long ntripServerConnectionAttemptTimeout = initialTimeout;
unsigned long nextConnectionAttempt = 0;
int numFailedNTRIPServerConnectionAttempts = 0;
unsigned long numSuccesfulUploads = 0;
unsigned long startSurvey = 0;
bool NTRIPConnected = false;
int ntrip_upload_period = 1000;
unsigned long next_ntrip_upload = 0;
...
void setup() 
{
    ...
    // USB serial
    Serial.begin(115200);
    //while (!Serial){}; // Pauses till serial starts. Remove if running apart from computer
...
void loop() 
{
    ...
    // Delay initial connection to NTRIP caster till after the self survey of the base station has completed
    if (survey_complete && !NTRIPConnected)
    {
        connectToNTRIP();
    }
    ...
    if (data_available)
    {
        ...
        // Upload correction data to NTRIP caster
        if (survey_complete == true)
        {
            bool uploadSuccess = uploadNTRIPData();
            Serial.print("Uploading data, num uploads = ");Serial.println(numSuccesfulUploads);
        }
    }
    ...
    if (millis() > next_update) 
    {
        ...
        events.send(String(numSuccesfulUploads).c_str(),"numSuccesfulUploads",millis());
...
void connectToNTRIP()
{
    if(millis() > nextConnectionAttempt)
    {
        ...
        // Attempt a connection, if it fails set a delay time till we try again
        if (!ntrip_s.subStation(host, httpPort, mntpnt,psw,srcSTR)) 
        {
            Serial.println("Mount failed");
            ntripServerLastConnectionAttempt = millis();
            ntripServerConnectionAttemptTimeout = ntripServerConnectionAttemptTimeout * 2.5;
            nextConnectionAttempt = ntripServerLastConnectionAttempt + ntripServerConnectionAttemptTimeout;
            numFailedNTRIPServerConnectionAttempts += 1;
            Serial.print("Next connection attempt: ");Serial.println(nextConnectionAttempt);
        }
        else
        {
            Serial.println("Subscribed to MountPoint");
            NTRIPConnected = true;
            ntripServerConnectionAttemptTimeout = initialTimeout;
            nextConnectionAttempt = millis();
            ntripServerLastConnectionAttempt = millis();
        }
    }
}
...
bool uploadNTRIPData()
{
    ...
    if (ntrip_s.connected())
    {
        ...
        ntrip_s.write((uint8_t*)rtcm_data,data_length);
        for(int i=0; i<data_length; i++)
        {
            Serial.print(rtcm_data[i], HEX);
        }
        uploadSuccess = true;
        Serial.println("");
    }
    // If the connection to the NTRIP caster is closed, try to reopen
    else if (millis() > nextConnectionAttempt)
    {
        ntrip_s.stop();
        ntripServerLastConnectionAttempt = millis();
        ntripServerConnectionAttemptTimeout = ntripServerConnectionAttemptTimeout * 2.5;
...
String init_rtk(const String& var)
{
    ...
    if(var == "NUM_UPLOADS")
    {
      return String(numSuccesfulUploads);
    }
