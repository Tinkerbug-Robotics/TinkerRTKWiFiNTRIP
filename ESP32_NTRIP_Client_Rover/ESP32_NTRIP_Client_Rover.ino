/**
 * Firmware for ESP32 running on the rover with correction data downloaded from NTRIP caster
 * on the internet using the ESP32 radio to connect to a WiFi network. The correction data from 
 * the NTRIP caster is then sent to the GNSS processor to compute an RTK solution.
...
void setup() 
{
    ...
    Serial.println("Connect to NTRIP caster");
    if(!ntrip_c.reqRaw(host,httpPort,mntpnt,user,passwd))
    {
        Serial.println("NTRIP Caster connnection failed");
        delay(15000);
    }
    Serial.println("NTRIP Caster connection succesful");
...
void loop() 
{
    ...
    // Grab data from the NTRIP caster and send to the GNSS receiver
